...
fit = dream(cobj, ~ group_id, as.data.frame(colData(pb)))
fit = eBayes(fit)

topTable(fit, coef='group_idstim', sort.by="none")
```


Partition variation in cell type composition
```{r ctc.crumblr.cp}
vp = fitExtractVarPartModel(cobj, ~ group_id, as.data.frame(colData(pb)))
