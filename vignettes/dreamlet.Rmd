...
# Run dreamlet differential expression
Since the normalized expression data and metadata are stored within `res.proc`, only the regression formula remains to be specified.  Here we only included the stimulus status, but analyses of larger datasets can include covariates and random effects.
...
```{r forrest}
plotForest( res.dl, coef = 'StimStatusstim', gene = 'IRF7')
```
...
# Testing differences in cell composition
...
Variance partioning analysis quanifyies the contribution of multiple sources of variation in cell type composition differences. 
```{r ctc.crumblr.cp}
